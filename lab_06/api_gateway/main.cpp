...
public:
                       ...
                       [[maybe_unused]] HTTPServerResponse &response)
    {
        ...
        std::string base_url_user = "http://lab_06_user:8080";
        std::string base_url_post = "http://lab_06_post:8080";
        std::string base_url_message = "http://lab_06_message:8080";
        std::string base_url_data;
        std::string service_name;

        if (hasSubstr(request.getURI(),"/post")||
            hasSubstr(request.getURI(),"/posts")) {
            base_url_data = base_url_post;
            service_name = "post";
        }
        else if (hasSubstr(request.getURI(),"/message")||
            hasSubstr(request.getURI(),"/messages")) {
            base_url_data = base_url_message;
